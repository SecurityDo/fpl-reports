...
** entries is a list of asset objects: {name, fqdn, model, platform, machineType, category, os, publicIP, privateIP, location, osVersion, vendor, uuid, serialNumber, region, vpc, account, sites, groups, tags}
...
** return an asset object {key, asset, updatedOn, providers}, if name match the asset name or fqdn.
** call this function in FPL event parser to get asset information. 
----
  let asset = Platform_Asset_Refresh(obj["@sender"])
  if asset {
    printf("asset key %s", asset.key)
  }
----
