...
** request:  {method, path, headers, paras, body, debug, encoding}
** header format: {header, value, variable}
** parameter format: {name, value, variable}
** encoding: "json" | "form" | "url"  (default is "json")
----
...
== Platform AWS api: call AWS API

* Platform_SQS_Send(integration, message)
** send message via "AWSSQS" integration.
** message:  json object
----
  Platform_SQS_Send("AIAssist", {
    event: "behaviorSummary",
    endpoints: ["fluencyAdmin"]
  })
----
...
** return site information // {siteURL, account, multiTenant, managerURL}
* Platform_Site_GetTenants()
** return tenant list // [{name, displayName, description}]
* Platform_Site_LoadTenantComponent(tenant)
** return tenant component list // [{id, name}]
...
* Platform_DataObject_Save(partition, props, data)
** Save data object to a partition. props is a property map for the data object
** return the data object id
* Platform_DataObject_Search(partition, (props) => {})
** Search data object from a partition. the search lambda function return true if the data object match the search criteria.
** return a list of data object ids
* Platform_DataObject_Get(partition, id)
** Get data object from a partition
** return {props, data} if found 
* Platform_DataObject_Delete(partition, id)
** Delete data object from a partition
----
let partition = "mydata"
let props = {name: "foo", count: 100}
let id = Platform_DataObject_Save(partition, props, {table1, stream2})
let ids = Platform_DataObject_Search(partition, (props) => {
  return props.name == "foo"
})
let {props, data} = Platform_DataObject_Get(partition, id)
Platform_DataObject_Delete(partition, id)
----
...
== Platform Resource API:  Resource Search and Lookup

* Fluency_ResourceLoad(vendor, resourceType, query, (obj, customer) => {})
* Fluency_ResourceLookup(vendor, resourceType, customer, key)
----
function main() {
  let {entry, exist} = Fluency_ResourceLookup("Office365", "user", "*", "foo@gmail.com")
  printf("exist: %t, value: %s", exist, entry)
