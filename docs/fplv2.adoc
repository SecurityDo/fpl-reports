...
.Table t generated from the function call
|===
|ID | Col1 | Col2 
...
* GroupBy(({col1, col2 ... }) => { keys:{groupByKey1,...}, columns:{ aggregates }} 
...
** count: {Count:true}
** available aggregate functions: count, sum, avg, min, max, values, coalesce, first, argmin, argmax

----
   let customerTable = bucketTable.GroupBy(({Customer, S3_Cost}) =>  {
      return { keys:{Customer}, columns:{sum:{S3_Cost}} }
   })
----

** argmax and argmin
*** argmax(colToCompare, colToReturn)
*** argmax will return the row with the max value of the colToCompare
*** argmin will return the row with the min value of the colToCompare
*** colToReturn can be more than one column separated by commas (e.g argmax(col1, col2, col3)

.Table t generated from the jsonTable function call
|===
|ID | Col1 | Col2 
...
let t = jsonTable(arr)
let tg = t.GroupBy( ({ID, Col1, Col2}) => {
    return { keys:{ID}, columns:{ argmax:{Col1, Col2} }}
})
// table tg only has one row with ID = "a" and Col1 = "1921" and Col2 = "mno" as "1921" is greater than "1900"
return {tg}
----
.Table tg generated from the GroupBy function call aggregated with argmax
|===
|ID | Col1 | Col2 
